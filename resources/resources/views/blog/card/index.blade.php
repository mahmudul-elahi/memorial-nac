<div class="card border-0 rounded-3">
    ...
    <a href="{{ route('blog.posts.show', $post->slug) }}">
        <div class="card-img-top img-responsive img-responsive-16by9 shadow" style="background-image: url({{ asset($post->getThumbnail()) }});"></div>
    </a>
    ...
    <span class="badge bg-cyan-lt">
        {{ Carbon\Carbon::create($post->created_at)->format('d M, Y') }}
    </span>
    @if ($post->category)
    ...
    @endif
        ...
        <cite class="text-muted">
            {{ Str::limit($post->description, 145) }}
        </cite>
